...
Example: you don't try scraping websites yourself — you read `directives/scrape_website.md`, come up with inputs/outputs, and then run `execution/scrape_single_site.py`.
...
Why this works: if you do everything yourself, errors compound.  
90% accuracy per step = 59% success over 5 steps.

The solution is to push complexity into deterministic code. That way you just focus on decision-making.
...
Example: you hit an API rate limit → you then look into the API → find a batch endpoint that would fix it → rewrite script to accommodate → test → update directive.
...
Directives are your instruction set and must be preserved (and improved over time, not temporarily used and then discarded).
...
`.tmp/`
- All intermediate files (dossiers, scraped data, temp exports). Never commit, always regenerated.
